...
{
    if (dictionaryInitWith!=nil) {
        [dictionaryInitWith release];
    }
    dictionaryInitWith=[[NSDictionary alloc] initWithDictionary:dictionary];
...
-(NSDictionary*)dictionary
{
    NSMutableDictionary *dic=[NSMutableDictionary dictionary];
    unsigned int procount=0;
    objc_property_t *property=class_copyPropertyList([self class], &procount);
    for (int i=0; i<procount; i++) {
        objc_property_t pro=property[i];
        NSString *name=[NSString stringWithCString:property_getName(pro) encoding:NSUTF8StringEncoding];

        NSString *value=[self stringValueForProperty:pro];
        if (value!=nil) {
            [dic setObject:value forKey:name];
        }
    }
...
-(id)stringValueForProperty:(objc_property_t)property
{
    if (property==nil) {
        return @"";//空值，返回
    }
